...
class FileProxy(ABC):
    ...
    def is_external_path(relative_path: str) -> bool:
        ...
        """
        from serena.util.external_path import EXTERNAL_FILE_PROXY_REGISTRY

        return EXTERNAL_FILE_PROXY_REGISTRY.find(relative_path) is not None

    @classmethod
    def from_project_relative_path(cls, project: "Project", relative_path: str) -> "FileProxy":
        from serena.util.external_path import EXTERNAL_FILE_PROXY_REGISTRY

        proxy_type = EXTERNAL_FILE_PROXY_REGISTRY.find(relative_path)
        if proxy_type is not None:
            return proxy_type.from_relative_path(relative_path, project)
