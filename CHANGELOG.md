...
* Language Servers:
  - Fix: C#: Serena's `workspace/configuration` handler answered every option whose name contains
    "navigate" with `false`, which also disabled `csharp_navigate_to_decompiled_sources`; go-to-definition
    could therefore not resolve into assemblies without sources (#1983)
